...
class MySQLAnnonceDAO extends DAO implements CrudInterface
{
    ...
    public function insert(Entity $annonce)
    {
        try {
            ...
            $stmt->bindValue(':user_id', $annonce->getUser()->getUserId());
            $stmt->bindValue(':rubrique_id', $annonce->getRubrique()->getRubriqueId());
...
//            return $lastId;
    ...
    public function delete(Entity $annonce)
    {
        try {
            $this->getCnx()->beginTransaction();
            $stmt = $this->getCnx()->prepare('DELETE FROM annonce WHERE annonce_id = :id');
    ...
    public function update(Entity $entity)//$id, $entete, $corps
    {
        try {
            ...
            $now  = new DateTime();
            $now->setTimezone(new DateTimeZone(self::TIMEZONE));
            $stmt->bindValue(':rubrique_id', $entity->getRubrique()->getRubriqueId());
    ...
    public function getByUser(Utilisateur $utilisateur)
    {
        try {
            $this->getCnx()->beginTransaction();
            $stmt = $this->getCnx()->prepare('SELECT * FROM annonce WHERE user_id = :userId');
    ...
    public function getByRub(Rubrique $rub)
    {
        try {
            $this->getCnx()->beginTransaction();
            $stmt = $this->getCnx()->prepare('SELECT * FROM annonce WHERE rubrique_id = :rubId');
    ...
    public function getAll()
    {
        try {
            $this->getCnx()->beginTransaction();
            $stmt = $this->getCnx()->query('SELECT * FROM annonce');
